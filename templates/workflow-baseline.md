<!-- The canonical daVinci workflow. Single source of truth. Stamped into every generated project's
     CLAUDE.md (below its project facts) and imported by this repo's own CLAUDE.md. -->
...
- Before committing to a design or pattern, research it like a senior developer would: check current
  ...
  guidance).
- When a better pattern is found, update this CLAUDE.md's architecture sections (and `knowledge/`
...
Before implementing a feature, integration, or pattern that's genuinely new to this codebase (a new
...
- Don't let this CLAUDE.md accumulate decision-by-decision detail — park specific decisions/lessons in
  the right `knowledge/` subfolder and leave only a one-line pointer.
